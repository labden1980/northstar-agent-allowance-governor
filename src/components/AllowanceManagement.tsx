...
const headingByFilter: Record<StatusFilter, string> = {
  all: "Allowance Management",
  active: "Active Allowances",
  revoked: "Revoked Allowances",
  expired: "Expired Allowances",
};

const getStatusDateLabel = (allowance: Allowance, status: AllowanceStatus): string => {
  const formattedExpiry = formatDate(allowance.expiryDate);

  if (status === "active") {
    return `Expires ${formattedExpiry}`;
  }

  if (status === "revoked") {
    return `Revoked · Original expiry ${formattedExpiry}`;
  }

  return `Expired ${formattedExpiry}`;
};

const getHistoricalEmptyMessage = (statusFilter: StatusFilter): string => {
  if (statusFilter === "revoked") {
    return "No revoked allowances match this filter.";
  }

  if (statusFilter === "expired") {
    return "No expired allowances match this filter.";
  }
    ...
    <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/60 sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-1 text-3xl font-black tracking-[-0.03em] text-slate-950">{headingByFilter[statusFilter]}</h2>
        ...
        {showHistoricalSection && (
          <div className="space-y-3">
            ...
            {historicalAllowances.length > 0 ? (
                      ...
                      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div>
                          ...
                          <p className="mt-2 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">
                            Limit {formatCurrency(allowance.weeklyLimit)} · Spent {formatCurrency(allowance.spent)} · {getStatusDateLabel(allowance, effectiveStatus)}
                          </p>
                        </div>
              ...
              <p className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">
                {getHistoricalEmptyMessage(statusFilter)}
              </p>
            )}
          </div>
        )}
