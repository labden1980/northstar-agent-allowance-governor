...
const navItems = ["Dashboard", "Allowances", "Spend Simulator", "Audit Trail", "Demo Guide"];

export function AppShell({ children, onResetDemo }: AppShellProps) {
  return (
    <div className="min-h-screen bg-slate-100 text-slate-950">
      <div className="mx-auto flex min-h-screen w-full max-w-[1600px] flex-col lg:flex-row">
        <aside className="border-b border-slate-200 bg-slate-950 px-5 py-5 text-white lg:sticky lg:top-0 lg:flex lg:h-screen lg:w-72 lg:shrink-0 lg:flex-col lg:border-b-0 lg:border-r lg:border-slate-800 lg:px-6 lg:py-6">
          <div className="flex items-start justify-between gap-4 lg:block">
            <div>
              <p className="text-2xl font-black tracking-[-0.04em] text-white">AgentLedger</p>
          ...
          <nav className="mt-6 flex gap-2 overflow-x-auto pb-1 lg:flex-col lg:overflow-visible" aria-label="AgentLedger sections">
            {navItems.map((item, index) => (
              <a
                key={item}
                href={index === 0 ? "#dashboard" : `#${item.toLowerCase().replace(/ /g, "-")}`}
                className={`whitespace-nowrap rounded-2xl px-4 py-3 text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-cyan-300 ${
                  index === 0
                    ? "border border-cyan-300/30 bg-cyan-300/10 text-cyan-100 shadow-[0_0_24px_rgba(34,211,238,0.10)]"
          ...
          <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/85 px-5 py-4 backdrop-blur sm:px-8">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                <a href="#demo-guide" className="hidden rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm transition hover:border-cyan-300 hover:text-cyan-700 focus:outline-none focus:ring-2 focus:ring-cyan-400 sm:inline-flex">Demo Guide</a>
